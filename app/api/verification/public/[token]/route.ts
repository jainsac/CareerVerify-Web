...
export async function GET(_:Request,{params}:{params:Promise<{token:string}>}) {
  ...
  return NextResponse.json({careerId:row.careerProfile.careerId,employments:row.careerProfile.employments.map((e: (typeof row.careerProfile.employments)[number])=>({organization:e.organization.name,designation:e.designation,department:e.department,joinedAt:e.joinedAt,leftAt:e.leftAt,status:e.status}))});
}