...
export async function POST(req:Request){
 ...
 const letter=await prisma.experienceLetter.upsert({where:{employmentRecordId:e.id},create:{employmentRecordId:e.id,documentRef,documentHash},update:{documentRef,documentHash,revokedAt:null}});
 ...
 return NextResponse.json({ok:true,letterId:letter.id,documentHash});
}
